...
for k = 1:length(t_1)   
    %ERROR DE POSICION ABSOLUTA
     h_error_Kinetic(:,k) = hd(1:3, k) - h_Kinetic(1:3, k);
     h_error_Dinamic(:,k) = hd(1:3, k) - h_Dinamic(1:3, k);
     h_error_MPC(:,k) =     hd(1:3, k) - h_MPC(1:3, k);
...
RMSE_Kinetic_x = sqrt(mean(h_error_Kinetic(1, :).^2));
RMSE_Kinetic_y = sqrt(mean(h_error_Kinetic(2, :).^2));
RMSE_Kinetic_z = sqrt(mean(h_error_Kinetic(3, :).^2));

% Calcular el RMSE para h_error_Dinamic
RMSE_Dinamic_x = sqrt(mean(h_error_Dinamic(1, :).^2));
RMSE_Dinamic_y = sqrt(mean(h_error_Dinamic(2, :).^2));
RMSE_Dinamic_z = sqrt(mean(h_error_Dinamic(3, :).^2));

% Calcular el RMSE para h_error_MPC
RMSE_MPC_x = sqrt(mean(h_error_MPC(1, :).^2));
RMSE_MPC_y = sqrt(mean(h_error_MPC(2, :).^2));
RMSE_MPC_z = sqrt(mean(h_error_MPC(3, :).^2));
...
e_pose_Kinetic = line(t_1(1,1:length(h_error_Kinetic(1,:))),h_error_Kinetic(1,:));
set(e_pose_Kinetic, 'LineStyle', '-', 'Color', C10, 'LineWidth', 1.1*lw);

e_pose_Dinamic = line(t_1(1,1:length(h_error_Dinamic(1,:))),h_error_Dinamic(1,:));
set(e_pose_Dinamic, 'LineStyle', '-', 'Color', C4, 'LineWidth', 1.1*lw);

e_pose_MPC = line(t_1(1,1:length(h_error_MPC(1,:))),h_error_MPC(1,:));
set(e_pose_MPC, 'LineStyle', '-', 'Color', C14, 'LineWidth', 1.1*lw);
...
%% Legend nomeclature
hLegend_3 = legend([e_pose_Kinetic, e_pose_Dinamic, e_pose_MPC],{'$\|\tilde{\mathbf{\eta}}\|_{Kinetic}$','$\|\tilde{\mathbf{\eta}}\|_{Dinamic}$','$\|\tilde{\mathbf{\eta}}\|_{MPC}$'},'fontsize',12,'interpreter','latex','Color',[255 255 255]/255,'Location','best','NumColumns',1,'TextColor','black');
 set(gca,'ticklabelinterpreter','latex',...
         'fontsize',1.3*fontsizeTicks)
%% Figure properties
ax_3 = gca;
...
% 
% Definir los valores de ISE para cada coordenada y conjunto de datos
%values_x = [ISE_Kinetic_x, ISE_Dinamic_x, ISE_MPC_x];
%values_y = [ISE_Kinetic_y, ISE_Dinamic_y, ISE_MPC_y];
%values_z = [ISE_Kinetic_z, ISE_Dinamic_z, ISE_MPC_z];

% Actualiza los vectores values_x, values_y, values_z con los valores de RMSE
values_x = [RMSE_Kinetic_x, RMSE_Dinamic_x, RMSE_MPC_x];
values_y = [RMSE_Kinetic_y, RMSE_Dinamic_y, RMSE_MPC_y];
values_z = [RMSE_Kinetic_z, RMSE_Dinamic_z, RMSE_MPC_z];

% Crear una matriz de barras con 3 grupos y 3 barras en cada grupo
data = [values_x; values_y; values_z];

% Etiquetas para las barras
labels = {'Kinetic', 'Dinamic', 'MPC'};
categories = {'x', 'y', 'z'};

% Crear el gráfico de barras segmentado

bar(data, 'grouped');
xlabel('Coordenada');
ylabel('Valor ISE');
%title('Valores ISE por Coordenada y Conjunto de Datos');
