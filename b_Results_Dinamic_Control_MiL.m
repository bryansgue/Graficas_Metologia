...
set(u_l, 'LineStyle', '-', 'Color', C7, 'LineWidth', 1.3*lw);
u_m = line(t(1,1:length(u(2,:))),u(2,:));
set(u_m, 'LineStyle', '-', 'Color', C14, 'LineWidth', 1.3*lw);
u_n = line(t(1,1:length(u(3,:))),u(3,:));
set(u_n, 'LineStyle', '-', 'Color', C15, 'LineWidth', 1.3*lw);

uc_l = line(t(1,1:length(uc(1,:))),uc(1,:));
set(uc_l, 'LineStyle', '--', 'Color', [C7, 0.5], 'LineWidth', 1.3*lw);
uc_m = line(t(1,1:length(uc(2,:))),uc(2,:));
set(uc_m, 'LineStyle', '--', 'Color', [C14, 0.5], 'LineWidth', 1.3*lw);
uc_n = line(t(1,1:length(uc(3,:))),uc(3,:));
set(uc_n, 'LineStyle', '--', 'Color', [C15, 0.5], 'LineWidth', 1.3*lw);

uc_l_real = line(t(1,1:length(u_MiL(1,:))),u_MiL(1,:));
set(uc_l_real, 'LineStyle', '-.', 'Color', [C7, 0.25], 'LineWidth', 1.3*lw);
uc_m_real = line(t(1,1:length(u_MiL(2,:))),u_MiL(2,:));
set(uc_m_real, 'LineStyle', '-.', 'Color', [C14, 0.25], 'LineWidth', 1.3*lw);
uc_n_real = line(t(1,1:length(u_MiL(3,:))),u_MiL(3,:));
set(uc_n_real, 'LineStyle', '-.', 'Color', [C15, 0.25], 'LineWidth', 1.3*lw);
...
%% Legend nomeclature
hLegend_4 = legend([u_l,u_m, u_n, uc_l,uc_m, uc_n, uc_l_real,uc_m_real, uc_n_real ],{'${\mu}_{l_{ref}}$','${\mu}_{m_{ref}}$','${\mu}_{n_{ref}}$'},'fontsize',12,'interpreter','latex','Color',[255 255 255]/255,'Location','best','NumColumns',1,'TextColor','black');
 set(gca,'ticklabelinterpreter','latex',...
         'fontsize',1.3*fontsizeTicks)
%% Figure properties
ax_4 = gca;
...
export_fig b_Results_Dinamic_Control_MiL.pdf -q101
